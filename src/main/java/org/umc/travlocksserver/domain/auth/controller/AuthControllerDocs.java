...
public interface AuthControllerDocs {
	...
	@Operation(summary = "이메일 인증 코드 발송 API", description = """
	...
	@Operation(summary = "이메일 인증 코드 확인 API", description = """
	...
	@Operation(summary = "이메일 인증 코드 재발송 API", description = """
	...
	@Operation(summary = "비밀번호 재설정 링크 전송 API", description = """
	...
	@Operation(summary = "비밀번호 재설정 토큰 유효성 검증 API", description = """
	...
	ResponseEntity<SuccessResponse<AuthPasswordResetVerifyResponseDTO>> verifyPasswordResetToken(
		@Parameter(description = "비밀번호 재설정 토큰", required = true) @RequestParam("token")
		String token);

	@Operation(summary = "비밀번호 재설정 확정 API", description = """
	...
	@Operation(summary = "Google OAuth2 로그인 API", description = """
	...
	@Operation(summary = "Naver OAuth2 로그인 API", description = """
	...
	@ApiResponses({
		...
		@ApiResponse(responseCode = "400", description = "code/state 오류 / 토큰 발급 실패 / 사용자 정보 조회 실패", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
		@ApiResponse(responseCode = "409", description = "이미 이메일로 가입된 계정 존재", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
	})
