...
public interface NotificationControllerDocs {
	...
	@Operation(summary = "SSE 토큰 발급 및 쿠키 저장 API", description = """
	...
	@Operation(summary = "SSE 알림 구독 연결 API", description = """
		서버-클라이언트 간 실시간 알림 전송을 위한 SSE 연결 API입니다.
		- "/api/v1/notifications/sse-token" API를 통해 발급받은 쿠키(SSE_TOKEN)가 있어야 인증이 완료됩니다.
		""")
	public SseEmitter subscribe(@AuthenticationPrincipal
	Long memberId);

	@Operation(summary = "알림 전체 삭제 API", description = """
		알림 전체 삭제 API입니다.
		""")
	...
	@Operation(summary = "알림 리스트 조회 API", description = """
		알림 리스트 조회 API입니다.
