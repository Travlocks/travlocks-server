...
public class TemplateRepositoryCustomImpl implements TemplateRepositoryCustom {
	...
	*/
	...
	private NumberExpression<Double> calculateTotalScore(
		...
		NumberExpression<Double> remixScore = log10(t.remixCount.coalesce(0).add(1).doubleValue())
			.multiply(REMIX_WEIGHT);
		NumberExpression<Double> favoriteScore = log10(t.favoriteCount.coalesce(0).add(1).doubleValue())
			.multiply(FAVORITE_WEIGHT);
